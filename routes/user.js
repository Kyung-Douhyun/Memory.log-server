...
router.post('/username', userController.userName.post); //유저 상태메세지 변경
router.post('/userinfo', userController.getUserInfo.post); // email 기준 유저 정보
router.post('/userinfobyid', userController.getUserInfoById.post); // id 기준 유저 정보
...
router.post('/profile', upload.single('img'), (req, res) => {
	...
	try {
		...
		// console.log(err);
		res.status(500).send(OverconstrainedError);
	}
});
